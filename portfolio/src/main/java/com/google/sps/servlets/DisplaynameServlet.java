...
public class DisplaynameServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    out.println("<h1>Set Display name</h1>");

    UserService userService = UserServiceFactory.getUserService();
    if (userService.isUserLoggedIn()) {
      String displayname = UserInfoUtil.getUserDisplayname(userService.getCurrentUser().getUserId());
      out.println("<p>Set your display name here:</p>");
      out.println("<form method=\"POST\" action=\"/displayname\">");
      out.println("<input name=\"displayname\" value=\"" + displayname + "\" />");
      out.println("<br/>");
